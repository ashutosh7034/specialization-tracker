    ...
    if ($name === '' || $email === '' || $password === '' || $department_id <= 0) {
        ...
        if ($check_result && mysqli_num_rows($check_result) > 0) {
            ...
            $sql = "INSERT INTO users (name, email, password, role, department_id)
                    VALUES ('$name', '$email', '$hashed', 'admin', $department_id)";
...
$admins_result = mysqli_query($conn, "SELECT u.id, u.name, u.email, d.name AS department
                                     FROM users u
...
?>
<h1 class="page-title">Super Admin -> Add Admin</h1>
...
<div class="table-wrap">
    <table>
        ...
        <tbody>
        <?php if ($admins_result && mysqli_num_rows($admins_result) > 0): while ($row = mysqli_fetch_assoc($admins_result)): ?>
            <tr>
